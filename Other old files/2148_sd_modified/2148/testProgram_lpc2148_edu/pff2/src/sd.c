...
/*
 * SPI port initialisation for SD card reading.
 */
CHAR sdInit (void)
{
	SHORT i = 100;
	...
	// Sending 'i' times the command to the initiator SPI port.
	do
	{
		sdCommand (0, 0, 0);
		resp = sdResp8b ();
	...
	// If the loop has exited due to sending 'i' commands and not
	// because an answer has been received.
	if (resp != 1)
	{
		if (resp == 0xff)
		{
	...
	// Waiting for a response (returns 0 on CMD1) that the device
